---
title: "Architecture Guardrails Library"
---

# Architecture Guardrails Library

This library contains the approved architecture guardrails for the organisation. Guardrails are standards, policies, patterns, and rules that guide how systems are built and integrated. Browse by category, search by keyword, or filter by tag.

**51 guardrails** across 5 categories — 50 approved, 1 under review. Last updated: 2026-04-22.

## Browse by category

| Category | Total | Approved | Under Review |
|---|---|---|---|
| [Governance](guardrails/governance/index.md) | 36 | 35 | 1 |
| [Security](guardrails/security/index.md) | 8 | 8 | 0 |
| [Cloud](guardrails/cloud/index.md) | 2 | 2 | 0 |
| [API](guardrails/api/index.md) | 4 | 4 | 0 |
| [Architecture](guardrails/architecture/index.md) | 1 | 1 | 0 |

## Recently updated

| ID | Title | Category | Last updated |
|---|---|---|---|
| [GUARD-ARCH-001](guardrails/architecture/GUARD-ARCH-001.md) | Digital Service Design Standard | Architecture | 2026-04-03 |
| [GUARD-CLOUD-001](guardrails/cloud/GUARD-CLOUD-001.md) | NZ Government Cloud First Policy | Cloud | 2026-04-03 |
| [GUARD-GOV-014](guardrails/governance/GUARD-GOV-014.md) | NZ Government Web Accessibility Standard 1.2 | Governance | 2026-04-03 |
| [GUARD-GOV-017](guardrails/governance/GUARD-GOV-017.md) | Risk Assessment for Public Cloud Services | Governance | 2026-04-03 |
| [GUARD-GOV-037](guardrails/governance/GUARD-GOV-037.md) | Responsible AI Guidance for Businesses | Governance | 2026-04-03 |
| [GUARD-SEC-008](guardrails/security/GUARD-SEC-008.md) | Information Assurance Standard — NZ Digital Government | Security | 2026-04-03 |
| [GUARD-API-001](guardrails/api/GUARD-API-001.md) | API Guidelines — Part A: API Concepts and Management 2022 | API | 2026-04-02 |
| [GUARD-API-002](guardrails/api/GUARD-API-002.md) | API Security Guidelines — Part B (NZ Digital Government 2022) | API | 2026-04-02 |
| [GUARD-API-003](guardrails/api/GUARD-API-003.md) | API Guidelines Part C: API Development 2022 | API | 2026-04-02 |
| [GUARD-API-004](guardrails/api/GUARD-API-004.md) | New Zealand API Standard | API | 2026-04-02 |
